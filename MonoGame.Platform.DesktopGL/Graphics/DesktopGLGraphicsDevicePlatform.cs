...
namespace Microsoft.Xna.Framework.DesktopGL.Graphics
{
	public class DesktopGLGraphicsDevicePlatform : IGraphicsDevicePlatform
	{
		...
		public void DrawUserIndexedPrimitives<T> (Microsoft.Xna.Framework.Graphics.PrimitiveType primitiveType, T[] vertexData, int vertexOffset, int numVertices, int[] indexData, int indexOffset, int primitiveCount) where T : struct
		{
			throw new NotImplementedException ();
		}

		public void DrawUserIndexedPrimitives<T> (Microsoft.Xna.Framework.Graphics.PrimitiveType primitiveType, T[] vertexData, int vertexOffset, int numVertices, short[] indexData, int indexOffset, int primitiveCount) where T : struct
		{
			throw new NotImplementedException ();
		}
		...
		internal BlendState _lastBlendState;
		internal DepthStencilState _lastDepthStencilState;
		internal RasterizerState _lastRasterizerState;
		...
		public void Setup()
		{
			...
			var wnd = (GameBackbone.Instance.Window as OpenTKGameWindow).Window.WindowInfo;
			...
			if (Context == null || Context.IsDisposed)
			{
				...
				var samples = 0;
				if (GameBackbone.Instance.graphicsDeviceManager.PreferMultiSampling)
				{
				...
				{
					GameBackbone.Instance.Log("Failed to create OpenGL context, retrying. Error: " +
						e.ToString());
					major = 1;
					minor = 0;
		...
		public DesktopGLGraphicsDevicePlatform (IBlendStatePlatform blend, IDepthStencilStatePlatform depthStencil, IRasterizerStatePlatform raster)
		{
			_lastBlendState = new BlendState(blend);
			_lastDepthStencilState = new DepthStencilState(depthStencil);
			_lastRasterizerState = new RasterizerState(raster);			

			clearDepthStencilState = new DepthStencilState(depthStencil) { StencilEnable = true };
		}
		...
		public void Initialize()
		{
			...
			this.BlendState.Platform.ApplyState(this, true);
			this.DepthStencilState.Platform.ApplyState(this, true);
			this.RasterizerState.Platform.ApplyState(this, true);            
		}

		private DepthStencilState clearDepthStencilState;
		...
		public void CreateRenderTarget(Texture renderTarget, int width, int height, bool mipMap, SurfaceFormat preferredFormat, DepthFormat preferredDepthFormat, int preferredMultiSampleCount, RenderTargetUsage usage)
		{
			var color = 0;
			var depth = 0;
			var stencil = 0;
		...
		private static GLPrimitiveType PrimitiveTypeGL(Microsoft.Xna.Framework.Graphics.PrimitiveType primitiveType)
		{
			switch (primitiveType)
			{
			case Microsoft.Xna.Framework.Graphics.PrimitiveType.LineList:
				return GLPrimitiveType.Lines;
			case Microsoft.Xna.Framework.Graphics.PrimitiveType.LineStrip:
				return GLPrimitiveType.LineStrip;
			case Microsoft.Xna.Framework.Graphics.PrimitiveType.TriangleList:
				return GLPrimitiveType.Triangles;
			case Microsoft.Xna.Framework.Graphics.PrimitiveType.TriangleStrip:
		...
		public void DrawIndexedPrimitives(Microsoft.Xna.Framework.Graphics.PrimitiveType primitiveType, int baseVertex, int startIndex, int primitiveCount)
		{
			ApplyState(true);
		...
		public void DrawPrimitives(Microsoft.Xna.Framework.Graphics.PrimitiveType primitiveType, int vertexStart, int vertexCount)
		{
			ApplyState(true);
