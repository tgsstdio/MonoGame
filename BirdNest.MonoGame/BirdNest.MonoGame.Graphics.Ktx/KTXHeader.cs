	...
	public class KTXHeader
	{
		...
		public void Populate(byte[] headerChunk)
		{
		   ...
		   no GL extension that would accept 3D array or cube array textures. */
			...
			Instructions.GlTarget = (uint) TextureTarget.Texture1D;
			Instructions.GenerateMipmaps = false;
			if (PixelHeight > 0)
			{
				Instructions.TextureDimensions = 2;
				Instructions.GlTarget = (uint) TextureTarget.Texture2D;
			}
			if (PixelDepth > 0)
			{
				Instructions.TextureDimensions = 3;
				Instructions.GlTarget = (uint) TextureTarget.Texture3D;
			}
			...
			if (NumberOfFaces == 6)
			{
				...
				{
					Instructions.GlTarget = (uint) TextureTarget.TextureCubeMap;
				}
				else
				{
			...
			{
				if (Instructions.GlTarget == (uint) TextureTarget.Texture1D)
				{
					Instructions.GlTarget = (uint) TextureTarget.Texture1DArray;
				}
				else if (Instructions.GlTarget == (uint) TextureTarget.Texture2D)
				{
					Instructions.GlTarget = (uint) TextureTarget.Texture2DArray;
				}
				else
				{
