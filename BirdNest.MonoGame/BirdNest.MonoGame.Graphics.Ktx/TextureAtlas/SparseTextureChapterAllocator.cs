...
namespace BirdNest.MonoGame.Graphics.Ktx
{
	public class SparseTextureChapterAllocator : ITextureChapterAllocator
	{
		...
		public ITextureChapter Generate (TextureCatalog catalog, GLImageType imageType, ImageDimensions dims, TextureTarget glTarget)
		{
			int textureId = GL.GenTexture ();

			if (glTarget == TextureTarget.Texture1D)
			{
			...
			else if (glTarget == TextureTarget.TextureCubeMap)
			{
