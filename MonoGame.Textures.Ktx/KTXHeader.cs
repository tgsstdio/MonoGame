...
namespace MonoGame.Textures.Ktx
{
	...
	public class KTXHeader
	{
		...
		private IKtxPlatform mPlatform;
		public KTXHeader (IKtxPlatform platform)
		{
			mPlatform = platform;
		}
		...
		public void Populate(byte[] headerChunk)
		{
		   ...
		   no GL extension that would accept 3D array or cube array textures. */
			...
			Instructions.GlTarget = mPlatform.GetTexTargetTexture1DEnumValue (); //TextureTarget.Texture1D;
			Instructions.GenerateMipmaps = false;
			if (PixelHeight > 0)
			{
				Instructions.TextureDimensions = 2;
				Instructions.GlTarget = mPlatform.GetTexTargetTexture2DEnumValue (); // TextureTarget.Texture2D;
			}
			if (PixelDepth > 0)
			{
				Instructions.TextureDimensions = 3;
				Instructions.GlTarget = mPlatform.GetTexTargetTexture3DEnumValue (); // TextureTarget.Texture3D;
			}
			...
			if (NumberOfFaces == 6)
			{
				...
				{
					Instructions.GlTarget = mPlatform.GetTexTargetCubeMapEnumValue (); // TextureTarget.TextureCubeMap;
				}
				else
				{
			...
			{
				Instructions.TextureDimensions = 2;
				Instructions.GlTarget = mPlatform.GetTexTargetTexture2DEnumValue (); //(uint) TextureTarget.Texture2D;
				PixelHeight = 1;
			}

			if (NumberOfArrayElements > 0)
			{
				if (Instructions.GlTarget == mPlatform.GetTexTargetTexture1DEnumValue() ) // (uint) TextureTarget.Texture1D)
				{
					Instructions.GlTarget = mPlatform.GetTexTargetTexture1DArrayEnumValue (); //(uint) TextureTarget.Texture1DArray;
				}
				else if (Instructions.GlTarget == mPlatform.GetTexTargetTexture2DEnumValue ()) // (uint) TextureTarget.Texture2D)
				{
					Instructions.GlTarget = mPlatform.GetTexTargetTexture2DArrayEnumValue(); // (uint) TextureTarget.Texture2DArray;
				}
				else
				{
