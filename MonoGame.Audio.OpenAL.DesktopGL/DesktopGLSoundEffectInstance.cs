...
namespace MonoGame.Audio.OpenAL.DesktopGL
{
	...
	public class DesktopGLSoundEffectInstance : BaseSoundEffectInstance
    {
        ...
        private IOALSoundBuffer soundBuffer;
        private IOpenALSoundController controller;
        ...
        /// </summary>
		protected override void PlatformInitialize(byte[] buffer, int sampleRate, int channels)
        {
            InitializeSound();
            BindDataBuffer(
                buffer,
				(channels == 2) ? OALAudioFormat.Stereo16Bit : OALAudioFormat.Mono16Bit,
                buffer.Length,
                sampleRate
			    );
        }
        ...
        internal void BindDataBuffer(byte[] data, OALAudioFormat format, int size, int rate)
        {
		...
		protected override void PlatformApply3D(AudioListener listener, AudioEmitter emitter)
        {
		...
		protected override void PlatformPlay()
        {
			...
			// Pan
			AL.Source (sourceId, ALSource3f.Position, Pan, 0, 0.1f);
			// Volume
			AL.Source (sourceId, ALSourcef.Gain, _alVolume);
			// Looping
			AL.Source (sourceId, ALSourceb.Looping, _looped);
			// Pitch
			AL.Source (sourceId, ALSourcef.Pitch, XnaPitchToAlPitch(mPitch));
		...
		protected override void PlatformStop(bool immediate)
        {
            if (hasSourceId)
            {
		...
		protected override void PlatformSetIsLooped(bool value)
        {
            _looped = value;
		...
		protected override bool PlatformGetIsLooped()
        {
            return _looped;
        }

		protected override void PlatformSetPan(float value)
        {
            if (hasSourceId)
                AL.Source(sourceId, ALSource3f.Position, value, 0.0f, 0.1f);
        }

		protected override void PlatformSetPitch(float value)
        {
            if (hasSourceId)
                AL.Source (sourceId, ALSourcef.Pitch, XnaPitchToAlPitch(value));
        }

		protected override SoundState PlatformGetState()
        {
            if (!hasSourceId)
